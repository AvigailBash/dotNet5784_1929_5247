        ...
        public static void inputForTask(int num, int Id = 0)
        {
            try
            {
                ...
                if (num == 2)
                {
                    ...
                    t = new BO.Task { id = Id, alias = Alias ?? temp!.alias, description = Description ?? temp!.description, status = status2 ?? temp!.status, dependencies = taskList ?? temp!.dependencies, createdAtDate = createdAtDate2 ?? temp!.createdAtDate, schedualedDate = SchedualedDate ?? temp!.schedualedDate, startDate = StartDate ?? temp!.startDate, forecastDate = ForecastDate ?? temp!.forecastDate, completeDate = CompleteDate ?? CompleteDate, requiredEffortTime = RequiredEffortTime ?? temp!.requiredEffortTime, deliverables = Deliverables ?? temp!.deliverables, remarks = Remarks ?? temp!.remarks, engineer = EngineerInTask ?? temp!.engineer, coplexity = Level ?? temp!.coplexity, isActive = IsActive };
                    //t.id
                ...
                else
                {
                    ...
                    t = new BO.Task() { id = Id, alias = Alias, description = Description, status = status1, dependencies = taskList, createdAtDate = createdAtDate1, schedualedDate = SchedualedDate, startDate = StartDate, forecastDate = ForecastDate, completeDate = CompleteDate, requiredEffortTime = RequiredEffortTime, deliverables = Deliverables, remarks = Remarks, engineer = EngineerInTask, coplexity = Level, isActive = IsActive };
                }
                if (num == 1)
                    s_bl.Task!.Create(t);
                else
